...
**Uwaga o wynikach, których bramka nie ma jak sprawdzić (29.08.2026):** obniżenie `required_approvals` do 1 nie domykało przypadku, w którym pomijają WSZYSTKIE boty. Wynik czysto tekstowy (bez zrzutu, bez testów funkcjonalnych, bez powtórki) dostawał zero zgód i zero zastrzeżeń, więc `passed` wychodziło fałszywe, a zadanie szło do właściciela jako "Wymaga decyzji" z uzasadnieniem `Zastrzeżenia: brak szczegółów`. Trzy takie zadania wystarczały, by `kacper_monitor.py` założył dodatkowe zadanie naprawcze "quality_gate zawodzi powtarzalnie", które samo wpadało w tę samą ścieżkę. Teraz `run_gate` zwraca `nothing_to_check`, a `runner_loop._decyzja_bramki` rozdziela cztery sytuacje: przeszło / nie było czego sprawdzić (zamknięcie z jawną adnotacją "bez automatycznej kontroli jakości", bez zadania dla człowieka) / zadanie źle postawione / realna wada (dalej eskalacja). Pominięcie ZE zastrzeżeniem (np. zadanie wizualne bez zrzutu) nadal idzie do człowieka. Testy: `runner_loop_smoke_test.py`, `validation_gate_smoke_test.py`.
