...
| `mailerlite_client.py` | Konektor MailerLite REST API (kampanie + statystyki) | ✅ **zweryfikowany na koncie produkcyjnym 22.08.2026**: `filter[since]` nie istnieje w REST MailerLite (był zgadywany, dawał 400) — naprawione, filtrowanie po dacie po stronie klienta, `get_campaigns_sent_since`/`get_campaign_stats` przetestowane na realnych kampaniach; `mailerlite_client_smoke_test.py` pokrywa logikę stronicowania bez sieci |
