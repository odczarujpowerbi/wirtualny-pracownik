...
| `machine_status_reporter.py` | Cogodzinny raport statusu maszyny (wersje narzędzi, historia bootstrapu, RAM) do Projectly przez `client.publish_status()`, zwracana wartość niesie dodatkowe pole `published` (bool) — porażka publikacji nie ginie cicho, trafia do logu i do wyniku widocznego w `job_scheduler` | ✅ przetestowany z i bez historii bootstrapu, plus `machine_status_reporter_smoke_test.py` (27.08.2026: `publish_status()==False` widoczne w logu i w `published`); wysyłka w trybie mock — czeka na zapowiedzianą dedykowaną funkcję MCP po stronie Projectly |
...
| `start-agent-dev.bat` + `register-task-dev.ps1` (analogicznie `-checker`/`-marketing`/`-zarzad`) | Autonomiczny start per rola: `job_scheduler.py` (runner co 30 s + monitoring + samo-weryfikacja) jako zadanie w Harmonogramie zadań Windows (wyzwalacz: przy zalogowaniu, bez admina). Checker (dostęp do repo) świadomie BEZ autostartu (29.08.2026, zużycie tokenów) — startuje wyłącznie ręcznie. `start-agent-all.bat` odpala wszystkie cztery naraz, w osobnych oknach. | — (rejestracja przez `powershell -File register-task-dev.ps1`) |
