...
| `task_feedback_requester.py` | Prosi o feedback po zamknięciu zadania: komentarz + zadanie w Projectly + mail. Pomija zadania założone przez sam mechanizm (`task_titles.py`), żeby nie napędzać się własnymi zadaniami | ✅ end-to-end na mocku, w tym idempotencja (nie pyta drugi raz o to samo zadanie, `runs/feedback_requested.json`); `task_feedback_requester_smoke_test.py` pilnuje braku pętli feedback→feedback |
| `task_titles.py` | Prefiksy tytułów zadań zakładanych przez mechanizm (`Wymaga decyzji:`, `Kontynuacja:`, `Feedback:`) i ich rozpoznawanie — używane przez `escalation.py` i `task_feedback_requester.py` | ✅ pokryty w `task_feedback_requester_smoke_test.py` |
