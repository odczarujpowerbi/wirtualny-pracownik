...
| `meta_task_guard.py` | Próg meta-zadań: rozpoznaje zadania założone przez agenta (eskalacja / prośba o feedback / kontynuacja) i buduje ich tytuły idempotentnie. Runner pomija zadania należące do człowieka, zamiast eskalować własną eskalację w kółko | ⚠️ test `meta_task_guard_smoke_test.py` napisany; uruchom go razem z `self_check.py` |
