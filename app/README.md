...
| `escalation.is_escalation_task` / `runner_loop` (fix pętli eskalacji) | Zadanie eskalacyjne wracające do kolejki agenta (przypisanie nierozwiązane na osobę → Projectly przypisuje wg tokenu, czyli do konta AI) było brane jako zwykła praca: model, bramka, brak materiału, ponowna eskalacja i `needs_approval` co cykl. Żywy przebieg: „Wymaga decyzji: Alert: stan maszyny wymaga sprawdzenia”, osiem domknięć pod rząd na jednym zadaniu. Teraz runner rozpoznaje je (znacznik `[eskalacja-dla-czlowieka]` w opisie albo przedrostek tytułu) i odkłada ze statusem `waiting_for_human`: zero wywołań modelu, zero komentarzy, zero zmian statusu w Projectly. | ✅ `escalation_guard_smoke_test.py` |
