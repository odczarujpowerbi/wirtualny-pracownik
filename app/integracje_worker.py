...
def _okres(task, tool, dzis=None):
    ...
    niż zamówił człowiek. `dzis` wstrzykiwane tylko w testach — bez tego okres
    zawsze liczy się od realnej daty (period_resolver.resolve domyślnie bierze
    date.today())."""
    okres = period_resolver.resolve(_tekst_zadania(task), dzis)
    if not okres:
        return None, _odmowa(
...
def raport_mailerlite(task, klient=None, dzis=None):
    okres, blad = _okres(task, "mailerlite_report", dzis)
    if blad:
        return blad
...
def podsumowanie_zanfia(task, klient=None, dzis=None):
    okres, blad = _okres(task, "zanfia_query", dzis)
    if blad:
        return blad
