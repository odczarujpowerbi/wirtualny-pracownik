...
class MailerLiteClient:
    ...
    def get_campaigns_sent_since(self, since_iso_date):
        """Zwraca listę wysłanych kampanii od danej daty. Endpoint/parametry
        do zweryfikowania z dokumentacją — patrz uwaga w docstringu modułu."""
        response = requests.get(
            f"{BASE_URL}/campaigns",
            headers=self._headers,
            params={"filter[status]": "sent", "filter[since]": since_iso_date},
            timeout=15,
        )
