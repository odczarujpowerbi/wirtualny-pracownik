...
#
# Uruchomienie:  powershell -ExecutionPolicy Bypass -File register-task-dev.ps1
...
# start-agent-dev.bat mieszka w KORZENIU repo, nie w tym folderze (instalacja/skrypty/)
# - dwa poziomy wyzej. Zla wersja tej linii (Join-Path $PSScriptRoot "start-agent-dev.bat")
# nigdy nie trafiala na plik, ale nikt tego nie zauwazyl, bo zadanie bylo juz
# zarejestrowane recznie/inaczej (naprawione 23.08.2026). Plik przemianowany
# z "start-agent.bat" 29.08.2026 dla spojnosci z checker/marketing/zarzad.
$repoRoot = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent
$batPath = Join-Path $repoRoot "start-agent-dev.bat"
